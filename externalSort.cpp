...
class LocalSink : boost::noncopyable  //表示每个文件
{
 public:
  explicit LocalSink(int idx, int nbuckets)
  {
    char buf[256];
    snprintf(buf, sizeof buf, "tempfile-%05d-of-%05d", idx, nbuckets);
    out.open(buf);
  }
...
void reduce(const int nbuckets) //将每个文件中相同的内容合并
{
  for (int i = 0; i < nbuckets; ++i)
  {
    ...
    char buf[256];
    snprintf(buf, sizeof buf, "counttempfile-%05d-of-%05d", i, nbuckets);
...
void merge(const int nbuckets)   //将合并后的多个文件按count从大到小输出至output.txt
{
...
{
  int nbuckets = 10;  // number of files to save,reduce
